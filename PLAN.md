...
### Un-pinned defects — resolved 2026-07-28

- [x] **LTL delta was priced against a pallet count, not annual cases.**
      Resolved in `879bb40`. `per_unit_delta` is a $/case figure but was
      multiplied by `annual_pallet_shipments_per_sku` — a unit mismatch.

      **The originally recorded diagnosis was wrong** and is kept here
      because it nearly drove a much larger, unnecessary change. It called
      for repricing the delta onto a pallet *weight* (8.60 cwt), which
      would have required generating a `ti`/`hi`/`cases_per_pallet` field
      that exists nowhere. But LTL bills per hundredweight, so the penalty
      falls on tonnage shipped and `cases_per_pallet` cancels out of the
      product entirely:

          (cases_per_pallet × case_lb/100) × rate_delta × pallets_per_year
            == (case_lb/100) × rate_delta × annual_cases

      `per_unit_delta` was therefore already correct; only the multiplier
      was wrong. No new physical field was introduced.

      The 52-vs-520 question was also the wrong axis: both are asserted
      pallet counts with no source, which is why they compounded. Annual
      cases are now derived from disclosed figures ($25M ÷ 50 SKUs ÷
      `wholesale_price_per_unit` ÷ `case_pack_qty`) — one calibratable
      price replacing two unsourced volumes.

      Hero LTL $20.28 → $4,231.89/yr; portfolio $17,533.48 → $208,310.87.
